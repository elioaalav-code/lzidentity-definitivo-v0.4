 ...
 *  ROUTER (hash-based)
 * =================================================================== */
const ROUTES = ["chat","wallet","markets","network","identity"];
...
function setActive(route){
  ...
  document.title = `CipherMesh · ${route} — dApp`;
  // each view's onEnter hook
  ONROUTE[route]?.();
}
...
async function fetchMarkets(){
  try {
    ...
    document.getElementById("marketsRows").textContent = "rate-limited by coingecko — try again in a moment.";
  }
}
...
function renderMarkets(){
  ...
  root.innerHTML = marketsRows.map(c => {
    ...
    return `<div class="row" data-id="${c.id}">
      ...
      <div class="coin"><img src="${c.image}" alt="" loading="lazy"/><div class="nm"><span class="s">${c.symbol.toUpperCase()}</span><span class="n">${c.name}</span></div></div>
...
console.log("%c  ┌─────────────────────────────┐", css);
console.log("%c  │  CIPHERMESH dApp · v2.0     │", css);
console.log("%c  │  routes: chat wallet markets │", css);
console.log("%c  │          network identity   │", css);
console.log("%c  └─────────────────────────────┘", css);
