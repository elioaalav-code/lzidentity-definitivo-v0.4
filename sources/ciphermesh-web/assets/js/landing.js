...
/* hero meta pseudo-live */
let peers = 73, block = 229841007;
setInterval(() => {
  peers += Math.floor((Math.random() - 0.5) * 3);
  peers = Math.max(60, Math.min(90, peers));
  block += 1;
  const m1 = document.getElementById("m1"); if (m1) m1.textContent = `${peers} peers nearby`;
...
console.log("%c  ┌─────────────────────────────┐", css);
console.log("%c  │  CIPHERMESH · v2.0 · cortex │", css);
console.log("%c  │  off the grid · by signal   │", css);
console.log("%c  └─────────────────────────────┘", css);
console.log("%c  ↳ landing online · open /app.html for the dApp", "color:#5eead4;font-family:Geist Mono,monospace;font-size:10px");
